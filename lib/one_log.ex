defmodule OneLog do
  @moduledoc ~S"""
  A logger backend that tries to make a single request have only one log.
  ...
  defp handle_io_reply({:error, error}, _) do
    raise "failure while logging OneLog messages: " <> inspect(error)
  end
  ...
  defp read_env do
    Application.get_env(:logger, __MODULE__, Application.get_env(:logger, :one_log, []))
  end
end
