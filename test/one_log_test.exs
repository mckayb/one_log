defmodule OneLogTest do
  ...
  setup_all do
    ...
    LoggerBackends.add(OneLog)

    on_exit(fn ->
      LoggerBackends.remove(OneLog)
  ...
  test "does not start when there is no user" do
    :ok = LoggerBackends.remove(OneLog)
  ...
  after
    {:ok, _} = LoggerBackends.add(OneLog)
  end

  test "may use another device" do
    LoggerBackends.configure(OneLog, device: :standard_error)
  ...
  test "configures format" do
    LoggerBackends.configure(OneLog, format: "$message [$level]")

    assert capture_log(fn -> Logger.debug("hello") end) =~ "hello [debug]"
  end

  test "configures metadata" do
    LoggerBackends.configure(OneLog,
  ...
  test "ignores crash_reason metadata when configured with metadata: :all" do
    LoggerBackends.configure(OneLog, format: "$metadata$message", metadata: :all)
    Logger.metadata(crash_reason: {%RuntimeError{message: "oops"}, []})
    assert capture_log(fn -> Logger.debug("hello") end) =~ "hello"
  end

  test "configures formatter to {module, function} tuple" do
    LoggerBackends.configure(OneLog, format: {__MODULE__, :format})
  ...
  test "configures metadata to :all" do
    LoggerBackends.configure(OneLog, format: "$metadata", metadata: :all)
    Logger.metadata(user_id: 11)
  ...
  test "provides metadata defaults" do
    ...
    LoggerBackends.configure(OneLog, format: "$metadata", metadata: metadata)
  ...
  test "configures level" do
    LoggerBackends.configure(OneLog, level: :info)

    assert capture_log(fn -> Logger.debug("hello") end) == ""
  end

  test "filter by notice" do
    LoggerBackends.configure(OneLog, level: :notice)
  ...
  test "configures colors" do
    LoggerBackends.configure(OneLog, format: "$message", colors: [enabled: true])

    assert capture_log(fn -> Logger.debug("hello") end) ==
             IO.ANSI.cyan() <> "hello" <> IO.ANSI.reset()

    LoggerBackends.configure(OneLog, colors: [debug: :magenta])
    ...
    LoggerBackends.configure(OneLog, colors: [info: :cyan])
    ...
    LoggerBackends.configure(OneLog, colors: [warning: :magenta])
    ...
    LoggerBackends.configure(OneLog, colors: [error: :cyan])
